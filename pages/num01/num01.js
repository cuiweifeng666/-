...
Page({
  ...
  question: function () {
    ...
    console.log(data);
    that.setData({
      que_t: data.data[0].title,
      que_d: data.data[0].answer,
    })
  ...
  nextQue: function () {
    ...
    console.log(select)
    if (select > -1) {
      var chooseData = data.data[0].answer[select-1].id;
      let sho = data.data[0].answer[select-1]
  ...
  ask: function (e) {
    ...
    var data = wx.getStorageSync('response');
     ...
     var percent = data.data[0].answer[ind - 1].percent;
     if (percent != 0) {
